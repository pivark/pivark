...
<html lang="zh-CN">
    ...
    <body class="pv-portal-site pv-tags-index-page">
        ...
        <!-- 区块：标签索引主栏（本页示范形态：pv-side-list--tags 作主内容 chip 云） -->
        <main aria-label="维度索引" class="pv-channel-main pv-portal-main">
            <div class="container">
                {pv:if name="tags"}
                <ul class="pv-side-list pv-side-list--tags tags-cloud" aria-label="标签云">
                    {pv:foreach name="tags" item="tag"}
                    <li>
                        <a href="{$tag.url}" class="pv-side-list__chip tag-item">{$tag.name}<span class="tag-count">({$tag.document_count})</span></a>
                    </li>
                    {/pv:foreach}
                </ul>
                {pv:else}
                <div class="text-center py-5 text-muted" role="status">
                    <i class="bi bi-tags display-4 d-block mb-3 opacity-50" aria-hidden="true"></i>
                    <p class="mb-0 fs-5">暂无标签</p>
                </div>
                {/pv:if}
                <!-- 区块：分页 -->
