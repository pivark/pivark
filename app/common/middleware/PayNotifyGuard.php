...
/** 支付异步回调：策略 pay.notify */
class PayNotifyGuard
{
    ...
    public function handle(Request $request, \Closure $next): Response
    {
        ...
        $policy = $this->rateLimitRegistry->resolve('pay.notify');
        if ($policy === null || empty($policy['enabled'])) {
            return $next($request);
        }

        $blocked = $this->rateLimitGateway->check('pay.notify', $request->ip() ?: 'unknown');
        if ($blocked !== null) {
